...
export class UploadsService {
  async uploadImages(files: Express.Multer.File[], folder: string): Promise<CLoudinaryImageResponseInterface[]> {
  ...
  async deleteImage(public_id: string): Promise<void> {
    try {
      ...
      throw new HttpException(`Error al eliminar la imagen: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}